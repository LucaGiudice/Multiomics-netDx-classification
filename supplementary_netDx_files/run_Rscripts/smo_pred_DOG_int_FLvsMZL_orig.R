...
source("/home/wr/DOG/measures/sim_WJ2_Gnorm.R")
source("/home/wr/sim_PEA.R")
source("/home/wr/sim_HOE.R")

#Build the path of the input and output files ----
setwd(sprintf(rootData,"DOG/env/data_multi/"))
rda_files = list.files(pattern="*.rda")
rda_files=rda_files[c(12,15,6)] ##dog_gex_DLBCLvsMZL.rda,dog_meth_DLBCLvsMZL.rda,dog_ent_cn_DLBCLvsMZL.rda
pred_dir=sprintf(rootData,"DOG/predictions/CN2_int_GEX_int_METH_%s")
...
for(f_count in 1:length(rda_files)){
 ...
 datList[[name_l]] <- geno;
  ...
  outDir=sprintf(pred_dir,"FLvsMZL")
...
simMetric="custom",customFunc=sim_WJ)
    }
   ...
   # create genetic nets
    if (!is.null(groupList[["meth"]])) {netList3 <- makePSN_NamedMatrix(dataList[["meth"]],
                                                                          ...
                                                                          netDir,writeProfiles=FALSE,
                                                                          sparsify=TRUE,verbose=TRUE,
                                                                          append=TRUE,numC=numCores,sparsify_edgeMax=.Machine$integer.max,sparsify_maxInt=.Machine$integer.max,cutoff=.Machine$double.eps,simMetric="custom",customFunc=sim_HOE)
    }
  ...
  #Remove from the set of pathways and from netList those that do not have any gene in geno matrix
  n_genes_in_pathway=sapply(netList,FUN=length)
  pathwayList=pathwayList[n_genes_in_pathway>4]
